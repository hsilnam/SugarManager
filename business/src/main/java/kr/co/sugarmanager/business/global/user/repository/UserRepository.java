...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query("select case when coalesce(count(u.nickname),0) > 0 then true else false end as check from UserEntity u where u.pk = :userPk")
    ...
    @Query("select case when coalesce(count(u1.nickname),0) > 0 then true else false end as checkgroup " +
            "from UserEntity u1 " +
